...
class Model {
    ...
    protected insert(values: params) {
        return new Promise((resolve, reject) => {
            const { command, params } = SQLPayload.generateInsertPayload(this.modelName, values)
    ...
    protected readByParams(values: params) {
        return new Promise((resolve, reject) => {
            const { command, params } = SQLPayload.generateReadByParamsPayload(this.modelName, values);
            connection.query(command, [params], (error, data) => {
                if (error) {
                    reject(error.message)
                } else {
    ...
    protected update(valuesSet: params, valuesWhere: params) {
        return new Promise((resolve, reject) => {
            let {command, paramsSet, paramsWhere} = SQLPayload.generateUpdatePaylaod(this.modelName, valuesSet, valuesWhere)
            connection.query(command, [...paramsSet, ...paramsWhere], (error, data) => {
                if (error) {
                    reject(error)
                } else {
                    resolve(data)
    ...
    protected delete(values: params) {
        return new Promise((resolve, reject) => {
            const { command, params } = SQLPayload.generateDeletePayload(this.modelName, values);
            connection.query(command, [...params], (error, data) => {
                if (error) {
                    reject(error)
                }
